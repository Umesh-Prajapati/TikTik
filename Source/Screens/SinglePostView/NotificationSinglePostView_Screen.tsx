...
const NotificationSinglePostView_Screen = ({ route, navigation }: any) => {
    ...
    const GlobalData = useSelector((state: any) => state.NotificationImagePost_Store);
    const whichTypeScreen = 'NotificationSinglePostView'
    ...
    function postLike() {
        ...
        dispatch(NotificationImagePost_Action([...clone]));
    }

    function postDelete() {
        dispatch(NotificationImagePost_Action([]));
        return;
        let temp = GlobalData
        let cut = temp.splice(index, 1);
        dispatch(NotificationImagePost_Action([...cut]));
    }
...
export default NotificationSinglePostView_Screen
